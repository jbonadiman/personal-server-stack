"""
note from me: had to take this directly from 
https://github.com/rclone/rclone/blob/master/librclone/python/rclone.py, since
it's not avaible as a PyPi package. Took the opportunity to make my own
refactors.
...
class Rclone:
    ...
    def __init__(
            self,
            shared_object: str=f"./librclone{'.dll' if os.name == 'nt' else '.so'}"):
        self.client = CDLL(shared_object)
        self.client.RcloneRPC.restype = RcloneRPCResult
        self.client.RcloneRPC.argtypes = (c_char_p, c_char_p)
        self.client.RcloneFreeString.restype = None
        self.client.RcloneFreeString.argtypes = (c_char_p,)
        self.client.RcloneInitialize.restype = None
        self.client.RcloneInitialize.argtypes = ()
        self.client.RcloneFinalize.restype = None
        self.client.RcloneFinalize.argtypes = ()
        self.client.RcloneInitialize()
    ...
    def rpc(self, method: str, params: dict):
        ...
        RcloneException.
        """
        parsed_method = method.encode("utf-8")
        parameters = json.dumps(params).encode("utf-8")

        resp = self.client.RcloneRPC(parsed_method, parameters)

        output = json.loads(resp.Output.value.decode("utf-8"))
        self.client.RcloneFreeString(resp.Output)
        status = resp.Status
        if status != 200:
